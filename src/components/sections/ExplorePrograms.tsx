...
import { ArrowRight, BarChart, Eye, Heart, Share2 } from "lucide-react";
...
type Tab = "business" | "design" | "mechanics";

// Get course data from categories
const getBusinessCourses = () => {
  const business = categories.find(cat => cat.slug === "business");
  return business?.courses || [];
};

const getDesignCourses = () => {
  const design = categories.find(cat => cat.slug === "design");
  return design?.courses || [];
};

const getMechanicsCourses = () => {
  const mechanics = categories.find(cat => cat.slug === "mechanics");
  return mechanics?.courses || [];
};

// Build tab content from actual course data
const tabContent: Record<Tab, Course[]> = {
  business: getBusinessCourses().slice(0, 5),
  design: getDesignCourses().slice(0, 5),
  mechanics: getMechanicsCourses().slice(0, 5),
};
...
export default function ExplorePrograms() {
  ...
  return (
    <section className="py-16 bg-[length:200%_200%] animate-gradient-slow" style={{ background: 'var(--gradient-bg-subtle)' }}>
      <div className="container-custom">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-3xl mx-auto">
            ...
            skills and career. Choose from business, design, mechanics, and more.
          </p>
        </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                  activeTab === tab
                    ...
                    : "bg-white/80 backdrop-blur-sm text-gray-800 hover:bg-gray-100 shadow-sm"
                }`}
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {tabContent[activeTab].map((course) => (
            <Card
              key={course.slug}
              className="overflow-hidden group border-0"
            >
              <div className="relative">
                <div className="aspect-w-16 aspect-h-10 relative overflow-hidden image-gradient-overlay">
                  <Image
                    src={course.image}
                    alt={course.title}
                    width={300}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-10">
                    <TooltipProvider>
